...
edit_yaml() {
    ...
    sed -i "s|use_valid_frame_range:.*|use_valid_frame_range: true|" "$yaml_file"  #use the full scene, no cuts
...
for idx in "${!DATASETS[@]}"; do
    ...
    for feat in "${FEATURES_LIST[@]}"; do
        for mag in "${MAG_LIST[@]}"; do
            ...
            python3 "$PY_SCRIPT" --yaml "$yaml_job" --run "$run_id" --out_dir "runs_mvsec_dt4_cut" &
